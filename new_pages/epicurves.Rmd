...
Précédemment, nous avons utilisé le paquet **incidence2** pour créer des épicurves. Des mises à jour récentes du paquet **incidence2** font qu'il n'est plus possible de créer des graphiques avancee avec le fonction `plot()` du paquet.  Nous présentons brièvement comment créer des épicourbes en utilisant l'ancien format du paquet, qui fonctionne avec les versions de R < 4.2. 
...
Pour *tracer* l'objet *incidence*, utilisez `plot()` sur le *nom de l'objet incidence*. En arriére-plan, la fonction `plot.incidence2()` est appelée, donc pour lire la documentation spécifique à **incidence2**, vous devez exécuter `plot.incidence2`.  
...
Voir la section plus haut sur cette page pour une méthode alternative pour tracer l'incidence cumulative avec **ggplot2** - par exemple pour superposer une ligne d'incidence cumulative sur une épicurve.  
